...
Two sources of data:
  ...
  2. live language stats from the GitHub API
        python scripts/radar.py --github Sungho-pk42ac -o assets/radar-langs
...
def from_github(user: str, token: str | None, limit: int, exclude: set[str],
    ...
    while True:
        ...
        if not repos:
            break
        owned = [repo for repo in repos
                 if not repo.get("fork") and not repo.get("archived")]

        def fetch_languages(repo):
            try:
                return _api(repo["languages_url"], token)
            except (urllib.error.HTTPError, urllib.error.URLError):
                return {}

        # Language endpoints are independent. A small pool keeps profiles with
        # many repositories from taking several minutes to refresh while staying
        # well below GitHub's normal authenticated concurrency expectations.
        with ThreadPoolExecutor(max_workers=min(6, max(1, len(owned)))) as pool:
            futures = [pool.submit(fetch_languages, repo) for repo in owned]
            language_sets = [future.result() for future in as_completed(futures)]
